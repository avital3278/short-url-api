...
app = FastAPI(
    title="Short URL API", description="Production-ready API for shortening URLs"
)
...
def create_link(
    ...
    current_email: str | None = Depends(get_optional_user),
):
    # Rate Limiting
    now = datetime.now()
    history = [
        t for t in request_history.get("guest", []) if (now - t).total_seconds() < 60
    ]
    if len(history) >= 5:
        raise HTTPException(
            status_code=429, detail="Too many requests. Please wait a minute."
        )
    ...
    if current_email and not payload.custom_alias:
        existing = (
            ...
            .filter(
                models.Link.url == str(payload.url),
                models.Link.owner_email == current_email,
            )
            .first()
        )
        if existing:
    ...
    if payload.custom_alias:
        ...
        existing_alias = (
            db.query(models.Link)
            .filter(models.Link.short_code == payload.custom_alias)
            .first()
        )
        if existing_alias:
    ...
    new_link = models.Link(
        short_code=code, url=str(payload.url), owner_email=current_email, clicks=0
    )
    db.add(new_link)
    db.commit()
    db.refresh(new_link)
...
@app.get("/users/links")
def get_my_links(
    db: Session = Depends(get_db), current_email: str = Depends(get_current_user)
):
